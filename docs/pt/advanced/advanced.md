...
BilingualPDF armazena em cache os textos traduzidos para aumentar a velocidade e evitar chamadas de API desnecessárias para conteúdos idênticos. Você pode usar a opção `--ignore-cache` para ignorar o cache de tradução e forçar uma nova tradução.
...
BilingualPDF oferece suporte à tabela de glossário. O arquivo da tabela de glossário deve ser um arquivo `csv`.
