...
Esegui il comando di traduzione nella riga di comando per generare il documento tradotto `example-mono.pdf` e il documento bilingue `example-dual.pdf` nella directory di lavoro corrente. Utilizza Google come servizio di traduzione predefinito. Ulteriori servizi di traduzione supportati possono essere trovati [QUI](https://github.com/OpenX123/BilingualPDF/blob/main/docs/ADVANCED.md#services).
...
<!-- Note: System prompt is currently not supported. See [this change](https://github.com/OpenX123/BilingualPDF/pull/637). -->
...
BilingualPDF memorizza nella cache i testi tradotti per aumentare la velocità ed evitare chiamate API non necessarie per contenuti identici. Puoi utilizzare l'opzione `--ignore-cache` per ignorare la cache delle traduzioni e forzare una nuova traduzione.
...
BilingualPDF supporta la tabella del glossario. Il file della tabella del glossario dovrebbe essere un file `csv`.
